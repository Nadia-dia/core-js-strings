...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) < 0) return str;
  return (
...
function countVowels(str) {
  let counter = 0;
  const regex = /^[aeiouyAEIOUY]/;
  for (let i = 0; i < str.length; i += 1) {
    if (regex.test(str.charAt(i))) counter += 1;
  }
  return counter;
}
...
function isPalindrome(str) {
  const newStr = str
    ...
    .replaceAll('!', '')
    .replaceAll('?', '');
  const reverseStr = reverseString(newStr);
...
function encodeToRot13(str) {
  ...
  const regex = /^[!?,;: ]/;
  for (let i = 0; i < str.length; i += 1) {
    if (regex.test(str.charAt(i))) result += str.charAt(i);
